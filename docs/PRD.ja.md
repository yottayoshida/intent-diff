...
現在のリポジトリは README のみで、プロダクト思想はすでに明確である。
...
- Rust: 配布性、単一バイナリ、git/diff処理、速度に強い。
- Go: GitHub Action/CLI配布が容易。
- TypeScript/Node: GitHub APIやAI SDK周辺は速いが、単一バイナリ配布は工夫が必要。
- Python: プロトタイプは速いが、CLI製品配布と速度では工夫が必要。

推奨:

- v0.1はRustまたはGoでCLI coreを作り、LLM adapterはHTTPで薄く実装する。
- 研究プロトタイプを優先するならPythonも可。ただし将来のGitHub Action配布を考えると、早めに単一バイナリ志向に寄せる。
